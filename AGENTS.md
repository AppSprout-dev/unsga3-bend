...
- **Parallel hot loops (landed):** `evaluate_all`, association (`associate_raw` / `with_counts` / `gather`), `map_norm` / `gather_objs`, non-dominated `is_dominated` / `split_walk` / `first_scan`, nearest-ref tree-min, niche filters / histograms, `col_min`/`col_max`/`pick_extremes`, and `stamp_asgs` use balanced `a b = f(lo) f(hi)` mid-splits. Tournament / SBX / mutation / last-front niching stay sequential so the same seed yields the same fronts.
