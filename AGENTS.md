...
- **PackageId `Unsga3` stays C# / NuGet / GitHub Packages only.** This tree is not a NuGet package. Hub version **0.1.2** is published (`bend src/lib.bend --publish`, content hash `0x527a2a4fa91b05a0250d7be0e11d232a`), not nuget.org / GitHub Packages. First hub publish was **0.1.0** (`0xcd07e24a626a62e74603d48f436cd679`).
...
- **Hub 0.1.2:** current content-hash package. Consumers `import 0x527a2a4fa91b05a0250d7be0e11d232a/lib.bend as Unsga3`. `bend --publish` printed `import 0x527a2a4fa91b05a0250d7be0e11d232a/lib.bend as Lib`. Checkout development still uses `import ./src/lib.bend as Unsga3`. GitHub **0.1.1** was docs/ab only and did not re-publish (`0xcd07e24a626a62e74603d48f436cd679` until this 0.1.2 publish). 0.1.2 includes the RankNicheDistance fix; A/B stays `PymooCompatible`. Tag `v0.1.2` after the release PR merges.
