...
- **Parallel hot loops (landed):** `evaluate_all`, association (`associate_raw` / `with_counts` / `gather`), `map_norm` / `gather_objs`, NDS `split_walk` / `first_scan`, niche filters / histograms, `col_min`/`col_max`/`pick_extremes`, and `stamp_asgs` use balanced `a b = f(lo) f(hi)` mid-splits. Inner `is_dominated` and nearest-ref stay sequential (short-circuit / already parallel per individual). Tournament / SBX / mutation / last-front niching stay sequential so the same seed yields the same fronts.
